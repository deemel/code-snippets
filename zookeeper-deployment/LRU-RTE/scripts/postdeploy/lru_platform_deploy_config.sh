...
function run_rteconfing_03
{
    typeset routine="${PGM_NAME}.run_rteconfing_03"
    typeset status

    status=$(chkpoint_status ${routine} $MS_STATUS_FILE)
    [ "$status" = "$OK" ] && return $SUCCESS_RC
    print_msg "Run ${LRU_SCRIPT_PATH}/postdeploy/030_RunRteConfig_03.py"
    run_cmd "${LRU_SCRIPT_PATH}/postdeploy/030_RunRteConfig_03.py /tmp/${shost}.$$.ini ${shost}" || {
       print_msg "ERROR: Failure from ${LRU_SCRIPT_PATH}/postdeploy/030_RunRteConfig_03.py."
       return 1
    }
...
# Run ${LRU_SCRIPT_PATH}/postdeploy/030_RunRteConfig_03.py
run_rteconfing_03 || error_exit
